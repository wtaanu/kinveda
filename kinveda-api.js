...
const KV = (() => {
  ...
  function requireAuth(expectedRole) {
    const user  = getUser();
    const token = getToken();
    // Include the role in the redirect so the correct tab is pre-selected
    const signinUrl = expectedRole
      ? `kinveda-signin.html?role=${expectedRole}`
      : 'kinveda-signin.html';

    // No credentials at all → sign in
    if (!user || !token) { window.location.href = signinUrl; return null; }
